...
public enum HtmlTagEnum {
	 ...
	 *
	 * <p>
	 * label 元素不会向用户呈现任何特殊效果。不过，它为鼠标用户改进了可用性。 如果您在 label元素内点击文本，就会触发此控件。
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * 注释：如果您希望验证包含框架的页面，请确保 <!DOCTYPE> 被设置为 "HTML Frameset DTD" 或者 "XHTML
	 * Frameset DTD" 。
	 */
	 ...
	 *
	 * <p>
	 * <img>中的 usemap 属性可引用 <map> 中的 id 或 name 属性（取决于浏览器），所以我们应同时向 <map> 添加 id 和
	 * name 属性。
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * 音频格式的 MIME 类型 格式 MIME-type MP4 video/mp4 WebM video/webm Ogg video/ogg
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * 标签定义及使用说明 <a> 标签定义超链接，用于从一个页面链接到另一个页面。
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * 在 HTML 4.01 中，<a> 标签既可以是超链接，也可以是锚。在 HTML5 中，<a> 标签是超链接，但是假如没有 href
	 * 属性，它仅仅是超链接的一个占位符。
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * ul标签定义无序列表。 将ul标签与li标签一起使用，创建无序列表。
	 */
	HTML_LIST_UL("<ul", "</ul"),
	 ...
	 *
	 * <p>
	 * ol 标签。ol标签定义了一个有序列表. 列表排序以数字来显示。
	 *
	 * <p>
	 * 使用li标签来定义列表选项。
	 *
	 ...
	 *
	 * <p>
	 * 当使用 <menu> 元素时，command 元素将作为菜单或者工具栏的一部分显示出来。但是，用 command
	 ...
	 * 您只能对每个表格定义一个标题。
	 *
	 * 提示：通常这个标题会被居中于表格之上。然而，CSS 属性 "text-align" 和 "caption-side"
	 * 能用来设置标题的对齐方式和显示位置。
	 *
	 *
	 * HTML5 不支持 align 属性。
	 ...
	 *
	 * <thead> 标签必须被用在以下情境中：作为
	 * <table>
	 * 元素的子元素，出现在 <caption>、<colgroup> 元素之后，<tbody>、 <tfoot> 和
	 * <tr>
	 * 元素之前。
	 *
	 * 注释：<thead> 元素内部必须包含一个或者多个
	 * <tr>
	 * 标签。
	 *
	 * 提示：<thead>、<tbody> 和 <tfoot> 元素默认不会影响表格的布局。不过，您可以使用 CSS
	 * 来为这些元素定义样式，从而改变表格的外观。
	 *
	 */
	 ...
	 * 通过使用这些元素，使浏览器有能力支持独立于表格表头和表格页脚的表格主体滚动。当包含多个页面的长的表格被打印时，表格的表头和页脚可被打印在包含表格数据的每张页面上。
	 *
	 * <tbody> 标签必须被用在以下情境中：作为
	 * <table>
	 * 元素的子元素，出现在 <caption>、<colgroup> 和 <thead> 元素之后。
	 */
	 ...
	 * 通过使用这些元素，使浏览器有能力支持独立于表格表头和表格页脚的表格主体滚动。当包含多个页面的长的表格被打印时，表格的表头和页脚可被打印在包含表格数据的每张页面上。
	 *
	 * <tfoot> 标签必须被用在以下情境中：作为
	 * <table>
	 * 元素的子元素，出现在 <caption>、<colgroup> 和 <thead> 元素之后，<tbody> 和
	 * <tr>
	 * 元素之前。
	 *
	 * 注释：<tfoot> 元素内部必须包含一个或者多个
	 * <tr>
	 * 标签。
	 *
	 * 提示：<thead>、<tbody> 和 <tfoot> 元素默认不会影响表格的布局。不过，您可以使用 CSS
	 ...
	 * 注释：只能在
	 * <table>
	 * 元素之内，在任何一个 <caption> 元素之后，在任何一个 <thead>、<tbody>、<tfoot>、
	 * <tr>
	 * 元素之前使用 <colgroup> 标签。
	 *
	 ...
	 *
	 * <p>
	 * <span> 标签没有固定的格式表现。当对它应用样式时，它才会产生视觉上的变化。如果不对 <span> 应用样式，那么 <span>
	 ...
	 *
	 * <p>
	 * 如果 async="async"：脚本相对于页面的其余部分异步地执行（当页面继续进行解析时，脚本将被执行） 如果不使用 async 且
	 * defer="defer"：脚本将在页面完成解析时执行 如果既不使用 async 也不使用
	 * defer：在浏览器继续解析页面之前，立即读取并执行脚本
	 */
	 ...
	 *
	 * <p>
	 * 而幸运的是，object 对象提供了解决方案。如果未显示 object 元素，就会执行位于 <object> 和 </object>
